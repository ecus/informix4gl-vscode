...
let workspaceFunctions = new Map();
let indexReady = false;
// ─── Parse all .4gl files in workspace for function definitions ───
function indexWorkspaceFunctions() {
    return __awaiter(this, void 0, void 0, function* () {
        workspaceFunctions.clear();
        const files = yield vscode.workspace.findFiles('**/*.4gl', '**/node_modules/**');
        for (const fileUri of files) {
            yield indexFile(fileUri);
        }
        indexReady = true;
    });
}
function indexFile(fileUri) {
    return __awaiter(this, void 0, void 0, function* () {
        let doc;
        try {
...
function getParamTypes(funcInfo) {
...
function buildSignatureLabel(funcInfo) {
...
class Informix4GLCompletionProvider {
...
class Informix4GLSignatureHelpProvider {
    ...
    buildSignatureHelp(funcInfo, activeParam) {
    ...
    buildBuiltinSignatureHelp(func, activeParam) {
        const help = new vscode.SignatureHelp();
...
// ─── HoverProvider ───
class Informix4GLHoverProvider {
...
// ─── DefinitionProvider ───
class Informix4GLDefinitionProvider {
    provideDefinition(document, position, _token) {
        ...
        if (funcInfo) {
            return new vscode.Location(vscode.Uri.file(funcInfo.file), new vscode.Position(funcInfo.line, 0));
        }
        return undefined;
    }
}
// ─── Document Symbol Provider (improved) ───
class Informix4GLDocumentSymbolProvider {
    provideDocumentSymbols(document, _token) {
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (funcMatch) {
                ...
                const sym = new vscode.DocumentSymbol(name, detail, kind, line.range, line.range);
                stack.push({ symbol: sym, endRegex: endFuncRegex });
                continue;
            }
            if (mainRegex.test(text)) {
                const sym = new vscode.DocumentSymbol('MAIN', '', vscode.SymbolKind.Function, line.range, line.range);
                stack.push({ symbol: sym, endRegex: endMainRegex });
                continue;
            }
            if (globalsRegex.test(text)) {
                const sym = new vscode.DocumentSymbol('GLOBALS', '', vscode.SymbolKind.Namespace, line.range, line.range);
            ...
            if (stack.length > 0) {
                ...
                if (pending.endRegex.test(text)) {
                    ...
                    const fullRange = new vscode.Range(new vscode.Position(startLine, 0), new vscode.Position(i, line.text.length));
                    pending.symbol = new vscode.DocumentSymbol(pending.symbol.name, pending.symbol.detail, pending.symbol.kind, fullRange, pending.symbol.selectionRange);
...
function setupFileWatcher(context) {
    ...
    watcher.onDidChange((uri) => __awaiter(this, void 0, void 0, function* () { yield indexFile(uri); }));
    watcher.onDidCreate((uri) => __awaiter(this, void 0, void 0, function* () { yield indexFile(uri); }));
...
function activate(context) {
    const langSelector = { scheme: 'file', language: '4gl' };
    // Index workspace on activation
    indexWorkspaceFunctions();
    // Document Symbol Provider (Outline)
    context.subscriptions.push(vscode.languages.registerDocumentSymbolProvider(langSelector, new Informix4GLDocumentSymbolProvider()));
    // Completion Provider (IntelliSense)
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider(langSelector, new Informix4GLCompletionProvider(), '.', '('));
    // Signature Help Provider (parameter hints)
    context.subscriptions.push(vscode.languages.registerSignatureHelpProvider(langSelector, new Informix4GLSignatureHelpProvider(), { triggerCharacters: ['(', ','], retriggerCharacters: [','] }));
    // Hover Provider
    context.subscriptions.push(vscode.languages.registerHoverProvider(langSelector, new Informix4GLHoverProvider()));
    // Definition Provider (F12 / Go to Definition)
    context.subscriptions.push(vscode.languages.registerDefinitionProvider(langSelector, new Informix4GLDefinitionProvider()));
